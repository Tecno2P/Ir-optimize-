...
volatile unsigned long s_restartAt = 0;
...
void setup() {
    ...
    otaMgr.onEnd([](bool success, const String& msg) {
        ...
        if (success) s_restartAt = millis() + 1200;
    });
...
// ─────────────────────────────────────────────────────────────
void loop() {
    if (s_restartAt != 0 && millis() >= s_restartAt) {  // fire when future timestamp reached
        s_restartAt = 0;
...
static void _ensureFile(const char* path, const char* defaultContent) {
...
static void ensureDefaultFiles() {
    _ensureFile("/ir_database.json",  "{\"buttons\":[]}");
    _ensureFile("/ir_pins.json",
        ...
        "]}");
    _ensureFile(CFG_FILE,
        "{\"apSSID\":\"IR-Remote\",\"apPass\":\"irremote123\","
        "\"apChannel\":1,\"apHidden\":false,"
        "\"staSSID\":\"\",\"staPass\":\"\",\"staEnabled\":false}");
    _ensureFile("/groups.json",      "{\"groups\":[]}");
    _ensureFile("/schedules.json",   "{\"schedules\":[]}");
    _ensureFile("/ntp_config.json",  "{\"tzOffset\":19800,\"dstOffset\":0}");
    _ensureFile(IR_AUTO_SAVE_FILE,   "{\"autoSave\":false}");
}
